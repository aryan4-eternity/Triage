...
This project extends **HarmonE** (MIT, copyright 2025 Hiya Bhatt), a published
self-adaptive MLOps loop for traffic-flow regression. The MAPE-K skeleton, the
inference loop, and the RAPL energy instrumentation are HarmonE's work. AegisML
...
> *HarmonE decides which model to run. AegisML decides what to do at all — where
...
## 2. HarmonE vs AegisML — mapping table

| Dimension | HarmonE (inherited) | AegisML (contributed) |
|---|---|---|
...
Two bugs in HarmonE are reproducibility findings in their own right:
...
The LSTM/linear energy gap in HarmonE's published results is therefore
...
HarmonE's published numbers as if they were the same measurement.
...
HarmonE's `plan_drift` is below its threshold and reports nothing.
...
while aggregate R² = 0.84. HarmonE, monitoring only aggregate metrics, sees
...
1. *"This extends HarmonE — MIT licensed, copyright 2025 Hiya Bhatt — which we
...
- The bug fixes are publishable independently of AegisML.
  "We reproduced HarmonE and found two measurement confounds" is a
...
- BUG-3's fix changes the energy profile. We report before-and-after;
  we do not compare our corrected numbers against HarmonE's published ones.
