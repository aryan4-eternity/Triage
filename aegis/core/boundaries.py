...
def compute_boundary(
    ...
    if mode_hint == "integral":
        ...
        # current_value IS the current threshold (passed from EnergyMetrics)
        # The threshold was already updated by compute_energy; we just report it.
        violated  = current_value > orig_thr
        hard_b    = hard_ceiling or orig_thr * 2.0
        severity  = float(np.clip(
            abs(current_value - orig_thr) / max(abs(hard_b - orig_thr), 1e-9),
            0.0, 1.0,
...
def _severity(
    ...
    Returns 0 if not violated or bounds undefined.
    """
    if bound is None or hard_bound is None:
        return 0.0
    denom = abs(hard_bound - bound)
    if denom < 1e-9:
        return 0.0
    raw = abs(value - bound) / denom
